# KBBQ Idle 프로젝트 체크리스트

이 문서는 Unity 런타임, 선택형 백엔드, WebGL 배포 표면을 빠르게 점검하기 위한 최소 검증 순서입니다.
...
엄격 모드(모든 게이트 필수):
```bash
STRICT_RUNTIME_GATE=1 tools/runtime_quality_gate.sh
```
...
## 5) 데모 전 점검
- `ApiConfig.asset`에서 기본값은 네트워크 OFF 유지(프로젝트 안전 기본값)
