# Conversion UX Model - KBBQ Idle WebGL (Unity 2022.3 LTS)
...
| Primary buyer or user | Casual game publishers, ad-supported mini-game operators, and playable portfolio reviewers. |
| Value wedge | Playable K-BBQ idle loop with WebGL hosting and ad-review path. |
...
| First action | Open the portfolio-fit note, confirm the successor project, then inspect Unity play loop works only if this domain is relevant. |
...
- Use Session length only as a discussion metric until fresh validation exists.
